...
public:
	...
	void printAllKeys(function<bool(const T&)> predicate) { printAllKeys(root, predicate); }
	void printAllKeys(BNode<T>* p);//, function<bool(const T&)>);	
	T* search(T x) { return search(root, x); }
	T* search(BNode<T>* p, T x);
	void printBetween(BNode<T>* p, T,T);
};
...
template <typename T>
void BTree<T>::printAllKeys(BNode<T>* ptr)//, std::function<bool(const T&)> predicate)
{
